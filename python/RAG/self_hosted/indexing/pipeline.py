...
def index_chunks(chunks: List[Chunk], previous_doc_id: str | None = None) -> List[str]:
    ...
    if detail.get("topic_fit") is not None and detail["topic_fit"] < 0.3:
        ...
        logger.warning(f"[RAG/self_hosted] doc_id={first_meta.doc_id} : {msg}")
    ...
    logger.info(f"[RAG/self_hosted] {len(chunks)} chunks indexés dans '{config.QDRANT_COLLECTION}' (relevance_score={score}).")
